...
> 定位：本报告只做**现状审查 + bug 清单**（「现在到哪了、有什么问题」）。产品目标与架构见 `方案设计.md`，分阶段执行计划见 `开发计划.md`。
> 评审坐标系：以 `方案设计.md` 描述的**最终目标形态（整机 AES67 软件栈）**为标尺，判断「哪些是必须补的主干、哪些是可延后的旁支」。

---

## 0. 一句话结论

引擎（`engine/`）是全项目最扎实的部分，能自洽地收发 RTP；M9 刚补上「常驻引擎 + 命名管道 + Qt 面板」骨架，**方向正是目标形态需要的地基**。但距离目标产品还差三件事：① M9 骨架有 2 个真 bug（死锁 + 生命周期）先得修好才能用；② 发出的流因 **SAP 地址 bug** 实际**订阅不到**，对外契约还没立住；③ 声道路由 / 混音总线 / DSP（产品的核心价值）尚未开始。驱动仍是 Scream 换皮，其 IOCTL / 共享内存通路是死的，但按计划本就后置，不属当前阻断。
...
| 模块 | 现状 | 判断 |
|---|---|---|
| `engine/` 主控 + RTP 收发 | ✅ 能自洽收发，架构清晰 | 产品主干，保留深耕 |
...
M9 提交 = `engine/pipe_server.*`（引擎侧命名管道）+ `aes67_engine.cpp` 挂 handler + `panel/`（Qt 面板 + `pipe_client`）。**架构方向完全正确**：面板不碰音频，只通过 `\\.\pipe\AES67Engine` 收发文本命令（STATUS/START/STOP/SET…），引擎是常驻服务——这正是目标架构的「常驻引擎 + 对外接口」雏形。`PipeServer` 本身写得规范（`OVERLAPPED` + `m_stopEvent` 优雅退出、`PIPE_TYPE_MESSAGE`、handler 回调解耦、`m_running` 用 atomic）。下面是必须修的问题。
