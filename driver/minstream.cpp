...
--*/
{
    ...
    if (m_pTimer) {
        ...
        ExFreePoolWithTag(m_pTimer, AES67_POOLTAG);
    }

    if (m_pDpc) {
        ExFreePoolWithTag( m_pDpc, AES67_POOLTAG );
    }
...
--*/
{
    ...
    if (NT_SUCCESS(ntStatus)) {
        m_pDpc = (PRKDPC) ExAllocatePoolWithTag(NonPagedPool, sizeof(KDPC), AES67_POOLTAG);
        if (!m_pDpc) {
    ...
    if (NT_SUCCESS(ntStatus)) {
        m_pTimer = (PKTIMER) ExAllocatePoolWithTag(NonPagedPool, sizeof(KTIMER), AES67_POOLTAG);
        if (!m_pTimer) {
...
--*/
{
    ...
    if (m_ksState != KSSTATE_RUN) {
        // AES67 does not validate the format.
...
--*/
{
    ...
    m_pvDmaBuffer = (PVOID) ExAllocatePoolWithTag(NonPagedPool, BufferSize, AES67_POOLTAG);
    if (!m_pvDmaBuffer) {
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
    } else {
...
--*/
{
    ...
    if ( m_pvDmaBuffer ) {
        ExFreePoolWithTag( m_pvDmaBuffer, AES67_POOLTAG );
        m_ulDmaBufferSize = 0;
    }
} // FreeBuffer
#pragma code_seg()
...
--*/
{
    ...
    // AES67 does not have need a physical DMA channel. Therefore it
