...
CIVSHMEMSaveData::CIVSHMEMSaveData() : m_pBuffer(NULL), m_ulOffset(0), m_ulSendOffset(0), m_fWriteDisabled(FALSE) {
    ...
    if (g_UseIVSHMEM) {
        ...
        m_pWorkItem = (PIVSHMEM_SAVEWORKER_PARAM)ExAllocatePoolWithTag(NonPagedPool, sizeof(IVSHMEM_SAVEWORKER_PARAM), AES67_POOLTAG);
        if (m_pWorkItem) {
    ...
    if (g_UseIVSHMEM) {
        ...
        if (m_ivshmem.initialized) {
            ...
            if (RequestMMAP()) {
                ...
                RtlZeroMemory(m_ivshmem.mmap.ptr, sizeof(IVSHMEM_AES67_HEADER));
            }
            ReleaseMMAP();
        }
        if (m_ivshmem.devObj) {
        ...
        if (m_pBuffer) {
            ExFreePoolWithTag(m_pBuffer, AES67_POOLTAG);
        }
    }
} // CIVSHMEMSaveData
...
NTSTATUS CIVSHMEMSaveData::Initialize(DWORD nSamplesPerSec, WORD wBitsPerSample, WORD nChannels, DWORD dwChannelMask) {
    ...
    if (RequestMMAP()) {
        PIVSHMEM_AES67_HEADER hdr = (PIVSHMEM_AES67_HEADER)m_ivshmem.mmap.ptr;

        RtlZeroMemory(hdr, sizeof(IVSHMEM_AES67_HEADER));
        m_ivshmem.writeIdx = 0;
        hdr->offset = m_ivshmem.offset = (UINT8)sizeof(IVSHMEM_AES67_HEADER);
    ...
    if (NT_SUCCESS(ntStatus)) {
        m_pBuffer = (PBYTE)ExAllocatePoolWithTag(NonPagedPool, m_ivshmem.bufferSize, AES67_POOLTAG);
        if (!m_pBuffer) {
