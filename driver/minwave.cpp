...
//=============================================================================
NTSTATUS CreateMiniportWaveCyclicAES67( 
...
--*/
{
    ...
    //
    if (NT_SUCCESS(ntStatus)) {
        stream = new (NonPagedPool, AES67_POOLTAG) CMiniportWaveCyclicStream(OuterUnknown);
    ...
    if (PropertyRequest->Verb & KSPROPERTY_TYPE_BASICSUPPORT) {
        ...
        if (NT_SUCCESS(ntStatus)) {
            if (PropertyRequest->Verb & KSPROPERTY_TYPE_GET) {
                ...
                pComponentId->Product   = PID_AES67;
                pComponentId->Name      = NAME_AES67_SIMPLE;
                pComponentId->Component = GUID_NULL; // Not used for extended caps.
                pComponentId->Version   = AES67_VERSION;
                pComponentId->Revision  = AES67_REVISION;
                ...
                if ((pKsFormat->DataFormat.MajorFormat == KSDATAFORMAT_TYPE_AUDIO) && (pKsFormat->DataFormat.SubFormat == KSDATAFORMAT_SUBTYPE_PCM) && (pKsFormat->DataFormat.Specifier == KSDATAFORMAT_SPECIFIER_WAVEFORMATEX)) {
                    ...
                    if ( ((pWfx->wBitsPerSample == 16) || (pWfx->wBitsPerSample == 24) || (pWfx->wBitsPerSample == 32)) &&
                        ...
                        if ((pWfx->wFormatTag == WAVE_FORMAT_PCM) && (pWfx->cbSize == 0)) {
                            ...
                            if ((pWfx->nChannels >= 1) && (pWfx->nChannels <= 8) && (pWfxT->dwChannelMask <= 0x07FF) && (pWfx->nChannels == 1 || pWfx->nChannels % 2 == 0)) {
                                ntStatus = STATUS_SUCCESS;
                            }
                        }
                    }
...
Routine Description:
  ...
  called whenever CMiniportWaveCyclicStreamAES67::m_pTimer triggers.
